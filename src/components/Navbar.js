        ...
        <div className="flex justify-between items-center max-h-[15vh]">
            ...
            <div className=" flex items-center py-3 relative">
                <ul className="hidden md:flex px-2 lg:px-5 ">
                    ...
                    <Link to="/login"><button  onClick={()=>{localStorage.removeItem("loginData")}} className="invisible sm:visible drop-shadow-xl font-['Manrope'] px-4 py-2 border rounded-lg font-bold text-[#003247] 
                            sm:text-l text-sm hover:border-[#004C7D] hover:border-1 bg-white hover:text-[#29ffff] transition duration-300 
                            ease-in-out">
                               { localStorage.getItem("loginData")?<span>Logout</span>:"Register/Login"}
                            </button></Link>
            </div>
        </div>
